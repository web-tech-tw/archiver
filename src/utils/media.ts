...
export async function saveReceivedImage(
    ...
    if (data instanceof Response || data instanceof Blob) {
        const arrayBuffer = await data.arrayBuffer();
        await Bun.write(targetPath, new Uint8Array(arrayBuffer));
    } else if (data instanceof ArrayBuffer) {
        await Bun.write(targetPath, new Uint8Array(data));
    } else {
        await Bun.write(targetPath, new Uint8Array(data.buffer, data.byteOffset, data.byteLength));
    }
